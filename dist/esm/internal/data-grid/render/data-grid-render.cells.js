...
import { getBothSpanBounds, getRowSpanBounds, getSkipPoint, getSpanBounds, walkColumns, walkRowsInCol, } from "./data-grid-render.walk.js";
const loadingCell = {
...
export function drawCells(ctx, effectiveColumns, allColumns, height, totalHeaderHeight, translateX, translateY, cellYOffset, rows, getRowHeight, getCellContent, getGroupDetails, getRowThemeOverride, disabledRows, isFocused, drawFocus, freezeTrailingRows, hasAppendRow, drawRegions, damage, selection, prelightCells, highlightRegions, imageLoader, spriteManager, hoverValues, hoverInfo, drawCellCallback, hyperWrapping, outerTheme, enqueue, renderStateProvider, getCellRenderer, overrideCursor, minimumCellWidth) {
    ...
    let handledRowSpans = undefined;
    let handledBothSpans = undefined;
    ...
    walkColumns(effectiveColumns, cellYOffset, translateX, translateY, totalHeaderHeight, (c, drawX, colDrawStartY, clipX, startRow) => {
        ...
        walkRowsInCol(startRow, colDrawStartY, height, rows, getRowHeight, freezeTrailingRows, hasAppendRow, skipPoint, (drawY, row, rh, isSticky, isTrailingRow) => {
            ...
            if (cell.rowSpan !== undefined && cell?.span !== undefined) {
                ...
                if (handledBothSpans === undefined)
                    handledBothSpans = new Set();
                if (!handledBothSpans.has(spanKey)) {
                    const area = getBothSpanBounds(cell?.rowSpan, cell?.span, drawX, drawY, c.width, row, getRowHeight, c, allColumns);
                    if (area !== undefined) {
                        cellY = area.y;
                        cellX = area.x;
            ...
            if (cell?.rowSpan !== undefined && !cell.span) {
                ...
                if (handledRowSpans === undefined)
                    handledRowSpans = new Set();
                if (!handledRowSpans.has(spanKey)) {
                    const area = getRowSpanBounds(cell?.rowSpan, drawX, drawY, c.width, row, getRowHeight);
                    if (area !== undefined) {
                        cellY = area.y;
                        cellHeight = area.height;
            ...
            const rowSpanIsHighlighted = cell.rowSpan !== undefined &&
                selection.rows.some(index => cell.rowSpan !== undefined && index >= cell.rowSpan[0] && index <= cell.rowSpan[1] //alloc
                );
